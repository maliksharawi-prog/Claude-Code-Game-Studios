...
*Cross-GDD sync, 2026-07-18: corrected the process-kill Edge Cases row —
`attempt_number` resets to 1 on relaunch (owned by `screen-flow.md` §7),
it is never incremented by this document; added a bug-repro honesty note.*
...
| App is backgrounded or killed by the OS mid-level | RNG Service state exists only in memory for the session's duration; it is never persisted to disk. If the process is killed, the session is abandoned — re-entering the level starts a **new** session. **Correction (2026-07-18 cross-GDD sync)**: `attempt_number` is not this document's own counter — it is owned in-memory by `screen-flow.md` §7, which resets it to `1` on every fresh level entry from the map. A force-quit relaunch is indistinguishable, to that counter, from a fresh map entry (Screen Flow's own in-memory state is discarded on relaunch, `screen-flow.md` T21), so the next entry after a kill is **attempt 1**, not an increment of whatever attempt was in progress; the resulting `master_seed` is whatever `attempt_number = 1` derives (Formula F1), not "the next value in a running sequence." | Keeps RNG Service dependency-free (no coupling to Save & Persistence) for MVP; matches genre convention — match-3 games typically don't resume mid-cascade after a hard process kill. **Bug-repro honesty note**: this means a logged `(level_id, attempt_number, seed)` bug-repro record can under-report the true number of attempts a player made across one or more force-quits (each post-kill relaunch resets to attempt 1, so two genuinely different attempts can both log as `attempt_number = 1` with different seeds). This does not weaken reproducibility — the logged `(level_id, attempt_number, seed)` tuple for any *one* logged occurrence remains fully sufficient to reconstruct that exact board bit-for-bit — it only means attempt-count telemetry is not a reliable "how many tries" metric across kills |
