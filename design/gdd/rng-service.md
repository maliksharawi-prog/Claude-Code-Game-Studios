...
|--------|-----------|----------------------|
| Match-3 Board Engine | Board Engine depends on RNG Service | Consumes `board-refill` stream for gravity/refill candy selection every cascade step; uses `fork_stream()` for the "no valid moves" reshuffle mechanic. **To be confirmed in `board-engine.md` when authored** (per the bidirectional-dependency rule). |
