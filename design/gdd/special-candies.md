...
| Scoring & Star Thresholds (`design/gdd/scoring-stars.md`, not yet authored) | Will depend on Special Candies | Expected to consume `special_activated`/`match_cleared`'s `cleared_pieces` counts and `trigger_source` to compute combo/cascade point bonuses — this document defines *which cells clear and why*, never a point value; that remains Scoring's exclusive scope. **Reciprocal note**: when authored, its Dependencies section must list this document. |
| Level Objective & Move-Limit System (`design/gdd/level-objectives.md`, not yet authored) | Will depend on Special Candies | Expected to consume per-color tallies from `match_cleared`/`special_activated`'s `cleared_pieces` for `collect_color` objectives, including cells cleared via this document's combos and passive chains — fully supported already by Board Engine's existing signal payloads (§ Detailed Rules 9's Harvest Observation Point framing applies identically here). **Reciprocal note**: when authored, its Dependencies section must list this document. |
