...
*Cross-GDD sync, 2026-07-18: Dependencies row "Verified gap, not yet
closed" resolved — `level-objectives.md` § Detailed Rules 9 now confirms
its `record_level_completion()` call.*
...
| Level Objective & Move-Limit System (`design/gdd/level-objectives.md`, #7, Revised — Revision 2) | World Map depends on it (soft, forward, indirect) | The system that triggers `record_level_completion()` on a win. World Map has no direct dependency on it — only on the resulting Save & Persistence state. **Gap closed (2026-07-18 cross-GDD sync)**: `level-objectives.md` § Detailed Rules 9 now explicitly confirms this call, strictly before `level_resolved` fires, on WIN only — this calling responsibility is now confirmed by its owning document. |
